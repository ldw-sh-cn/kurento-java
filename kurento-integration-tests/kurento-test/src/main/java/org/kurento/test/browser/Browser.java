...
public class Browser implements Closeable {
    ...
    private void calculateUrl() {
        if (url == null) {
            if (protocol == Protocol.FILE) {
                ...
                try {
                    url = new URI(protocol.toString()
                            + webPageFile.getAbsolutePath());
                } catch (URISyntaxException e) {
                    throw new KurentoException(
                            "Exception generating URI from " + protocol
                                    + " and " + webPageFile.getAbsolutePath());
                }
            } else {
                ...
                log.debug(
                        "Protocol: {}, Hostname: {}, Port: {}, Web page type: {}",
                        protocol, hostName, serverPort, webPageType);

                try {
                    url = new URI(protocol.toString(), null, hostName,
                            serverPort, webPageType.toString(), null, null);
                } catch (URISyntaxException e) {
                    throw new KurentoException("Exception generating URI from "
                            + protocol + ", " + hostName + ", server port "
                            + serverPort + " and webpage type " + webPageType);
                }
            }
        }
    }
    ...
    private void createChromeBrowser(DesiredCapabilities capabilities)
        ...
        if (!usePhysicalCam) {
            ...
            if (video != null && (isLocal() || isDocker() || isElastest())) {
                ...
                log.debug(
                        "Using video {} in browser {} (exists {}, {} bytes, can read {})",
                        video, id, videoFile.exists(), videoFile.length(),
                        videoFile.canRead());
                options.addArguments(
        ...
        if (this.getBrowserType() != BrowserType.IEXPLORER) {
            ...
            try {
                File pageFile = new File(this.getClass().getClassLoader()
                        .getResource(kurentoTestPath).getFile());
                kurentoTestJsContent = new String(
                        Files.readAllBytes(pageFile.toPath()));
            } catch (NoSuchFileException nsfe) {
                InputStream inputStream = this.getClass().getClassLoader()
                        .getResourceAsStream(kurentoTestPath);
            ...
            // String recordingJs = "var
            // recScript=window.document.createElement('script');";
            // recordingJs += "recScript.type='text/javascript';";
            // recordingJs +=
            // "recScript.src='https://cdn.webrtc-experiment.com/RecordRTC.js';";
    ...
    public static class Builder {
        ...
        private int serverPort = getProperty(TEST_PORT_PROPERTY, getProperty(
                TEST_PUBLIC_PORT_PROPERTY, WebServerService.getAppHttpsPort()));
